/**
 * Catálogo administrativo del Perú (departamento → provincias → distritos).
 *
 * Portado 1:1 de `mobile/lib/core/constants/peru_locations.dart` para que el
 * selector de ubicación del onboarding web use EXACTAMENTE el mismo catálogo
 * (mismas formas canónicas con acentos) que el móvil → el backend resuelve la
 * misma localidad y los filtros de ubicación encuentran al proveedor.
 */
...
export const PERU_DISTRICTS: Record<string, string[]> = {
  'Huancayo': ['Huancayo', 'El Tambo', 'Chilca', 'Huancán', 'Viques', 'Pilcomayo', 'Sapallanga', 'Chupaca', 'Cajas', 'Chongos Alto', 'Cullhuas', 'Huacrapuquio', 'Ingenio', 'Palian', 'Pucará', 'Quilcas', 'San Agustín de Cajas', 'San Jerónimo de Tunán', 'Saño', 'Sicaya', 'Santo Domingo de Acobamba'],
  ...
  'Concepción': ['Concepción', 'Aco', 'Andamarca', 'Chambará', 'Cochas', 'Comas', 'Manzanares', 'Mariscal Castilla', 'Matahuasi', 'Mito', 'Nueve de Julio', 'Orcotuna', 'San José de Quero', 'Santa Rosa de Ocopa'],
...
export function provincesOf(department: string | null | undefined): string[] {
  return department ? PERU_PROVINCES[department] ?? [] : [];
}

export function districtsOf(province: string | null | undefined): string[] {
  return province ? PERU_DISTRICTS[province] ?? [] : [];
}
