...
void main() {
  ...
  group('UserModel', () {
    ...
    test(
      'role desconocido NO se normaliza — se preserva tal cual lo manda backend',
      () {
    ...
    test(
      'copyWith preserva email + id (no son sobrescribibles) y respeta defaults',
      () {
    ...
    test(
      'toJson incluye campos clave (no incluye isEmailVerified — no es persistido)',
      () {
        const u = UserModel(
          id: 1,
          email: 'x@y',
  ...
  group('ChatMessageModel + MessageStatus', () {
    ...
    test('createdAt malformado cae a DateTime.now sin lanzar', () {
      ...
      expect(
        m.createdAt.isAtSameMomentAs(before) || m.createdAt.isAfter(before),
        true,
      );
    ...
    test(
      'apiName mapea correctamente — sending y failed nunca se mandan como otra cosa que SENT',
      () {
    ...
    test(
      'key identidad: usa id real si existe, si no clientTempId, si no fallback timestamp',
      () {
  ...
  group('ChatRoomSummary', () {
    Map<String, dynamic> roomJson({Map<String, dynamic>? lastMessage}) => {
      'id': 100,
      'clientId': 1,
      'providerId': 2,
  ...
  group('ProviderModel', () {
    test(
      'lee categoryName desde providerCategories (M:N) y fallback legacy',
      () {
    ...
    test('enums caen al default cuando viene un valor desconocido', () {
      expect(
        AvailabilityStatus.fromString('SUPER_DISPONIBLE'),
        AvailabilityStatus.disponible,
      );
      expect(
        AvailabilityStatus.fromString('OCUPADO'),
        AvailabilityStatus.ocupado,
      );
      expect(
        AvailabilityStatus.fromString('CON_DEMORA'),
    ...
    test(
      'locationLabel: OFICIO solo distrito; NEGOCIO provincia + distrito',
      () {
    ...
    test('copyWith(availability) actualiza el badge (sync realtime P4)', () {
      final p = ProviderModel.fromJson({
        'id': 1,
        'businessName': 'X',
        'phone': '999',
      ...
      expect(p.availability, AvailabilityStatus.disponible);
      final busy = p.copyWith(availability: AvailabilityStatus.ocupado);
      expect(busy.availability, AvailabilityStatus.ocupado);
      expect(busy.id, 1); // resto intacto
      // Sin pasar availability se conserva el actual.
      expect(
        p.copyWith(isFavorite: true).availability,
        AvailabilityStatus.disponible,
      );
    });
  });
  ...
  group('ServiceRequestModel + OfferModel + OpportunityModel', () {
    ...
    test(
      'OfferModel: status desconocido cae a pending; avatar fallback a image',
      () {
    ...
    test(
      'OpportunityModel.isFull es true cuando offersCount alcanza maxOffers',
      () {
  ...
  group('AppNotification', () {
    ...
    test('fromSocket usa avatarUrl o senderAvatarUrl', () {
      final n = AppNotification.fromSocket({
        ...
        'body': 'hola',
        'senderAvatarUrl': 'https://avatar/x',
      });
      ...
      final r = ReferralReward.fromJson(<String, dynamic>{
        ...
        'provider': <String, dynamic>{
          ...
          'providerCategories': <dynamic>[
            <String, dynamic>{
              'category': <String, dynamic>{'name': 'Belleza'},
            },
          ],
        },
      });
