...
/// Catálogo PÚBLICO de un proveedor (vista cliente), agrupado por sección,
/// con carrito (pedido de varios productos en un solo WhatsApp).
class CatalogScreen extends StatefulWidget {
  const CatalogScreen({super.key, required this.providerId, this.businessName});
...
class _CatalogScreenState extends State<CatalogScreen> {
  ...
  late Future<ApiResult<CatalogResponse>> _future;
  final Map<int, int> _cart = {};

  @override
  void initState() {
    super.initState();
    _future = _repo.getCatalog(widget.providerId);
  }
  ...
  List<CartLine> _lines(Map<int, CatalogProductModel> byId) => _cart.entries
  ...
  Future<void> _order(String? number, List<CartLine> lines) async {
    final url = buildOrderUrl(
      number: number,
      businessName: widget.businessName ?? 'la tienda',
      lines: lines,
    );
    if (url == null) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.businessName ?? 'Catálogo'),
        backgroundColor: AppColors.bgCard,
      ),
      body: FutureBuilder<ApiResult<CatalogResponse>>(
        future: _future,
        builder: (context, snap) {
          ...
          if (result == null || result.isFailure) {
            return _ErrorState(
              message: result?.errorMessage ?? 'No se pudo cargar el catálogo',
              onRetry: _reload,
            );
          }
          ...
          final byId = {for (final it in catalog.allItems) it.id: it};
          final number = catalog.allItems
          ...
          child: ElevatedButton(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  'Pedir $count ${count == 1 ? 'producto' : 'productos'} · S/ ${total.toStringAsFixed(2)}',
                  style: const TextStyle(
                    fontWeight: FontWeight.w800,
                    fontSize: 15,
                  ),
    ...
    return const Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 12),
          Text(
            'Esta tienda aún no publicó su catálogo.',
