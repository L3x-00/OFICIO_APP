...
- **Asistente WhatsApp F1–F4 con OpenWA** (PR #69, squash `0993337`, 2026-08-13): módulo `backend/src/whatsapp-assistant/`, aislado de Ofi. F1 verifica HMAC sobre cuerpo crudo, sesión, canal y deduplicación; aplica FAQ pública Servi, rechazo fuera de alcance, STOP y HUMANO. F2 usa Ofi solo para FAQ y búsqueda pública de proveedores, sin herramientas ni acciones de escritura. F3 vincula el número mediante OTP HMAC de 10 minutos y resuelve el contexto fresco por mensaje. F4 aporta handover y DLQ operativos bajo ADMIN, sin PII, replay ni reintento. Nunca persiste texto, teléfono, JID ni messageId en claro. Los SQL idempotentes `whatsapp_assistant.sql`, `whatsapp_assistant_links.sql` y `whatsapp_assistant_operations.sql` fueron aplicados manualmente por el propietario. F0 OpenWA fue configurado por el propietario. **Todas las flags continúan apagadas: no activar ni ejecutar smoke con salida hasta orden explícita del propietario.** Runbook: `docs/INTEGRACION_OPENWA_SERVI.md`.
