...
- **Graphify 2026-08-13:** regenerado desde worktree limpio en `110e807` (PR #69): 1,040 archivos, 10,043 nodos, 16,425 relaciones y 487 comunidades (368 mostradas; 119 delgadas omitidas). Extracción 100%; 74 relaciones inferidas con confianza promedio 0.80. Incluye `whatsapp-assistant` F1–F4. `graph.html` no se genera porque supera el límite visual de 5,000 nodos; usar `GRAPH_REPORT.md`, `graph.json` y `graphify query`. El grafo excluye cambios locales sin commit; commits solo documentales/tooling no invalidan su frescura.
