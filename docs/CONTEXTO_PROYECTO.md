...
**Última actualización:** 2026-08-12 · Estado: **Captación de negocios con botón en el panel Admin + credenciales al convertir, mergeado a `main`** — PRs #61 (`cffc197`) y #62 (`94152d2`), CI verde. El botón "Captación" del admin convierte leads del staging en `Provider` (⚠️ nueva superficie de prod: `LeadConversionModule` ahora SÍ está en `AppModule`); al convertir genera email `<negocio>@oficioapp.org.pe` + contraseña de 8 chars mostrada una sola vez. El scraper (repo `Servi-Scrapping`, `27dec06`) corrigió las fotos (una por negocio + HD). El SQL de staging ya está aplicado en Supabase. Servicios Profesionales V1 (PR #55) sigue en prod. Subastas, ofertas, referidos/monedas, agenda y cotización continúan OCULTAS de forma reversible; Carta y Catálogo siguen visibles solo para proveedores NEGOCIO.
...
- **Graphify 2026-08-12:** regenerado y validado desde worktree limpio contra `fb11bca`: 1,012 archivos, 9,850 nodos, 16,018 relaciones y 480 comunidades (366 mostradas; 114 delgadas omitidas). Extracción 100%; 74 relaciones inferidas con confianza promedio 0.80. `graph.html` no se genera porque supera el límite visual de 5,000 nodos; usar `GRAPH_REPORT.md`, `graph.json` y `graphify query`. El grafo excluye cambios locales sin commit; commits solo documentales/tooling no invalidan su frescura.
