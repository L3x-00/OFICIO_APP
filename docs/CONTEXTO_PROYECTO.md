...
- **Graphify 2026-08-13:** regenerado desde `main` limpio en `e8bc4a1` (PR #71): 1,094 archivos, 10,022 nodos, 16,443 relaciones y 464 comunidades (378 mostradas; 86 delgadas omitidas). Extracción 100%; 75 relaciones inferidas con confianza promedio 0.80. Incluye `whatsapp-assistant` F1–F4 y la auditoría de Ofi. `graph.html` no se genera porque supera el límite visual de 5,000 nodos; usar `GRAPH_REPORT.md`, `graph.json` y `graphify query`. El grafo excluye cambios locales sin commit; commits solo documentales/tooling no invalidan su frescura.
