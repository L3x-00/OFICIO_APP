...
- **Graphify 2026-08-13:** regenerado desde `main` limpio en `43fb9dd` (PR #73): 1,095 archivos, 10,031 nodos, 16,459 relaciones y 470 comunidades (381 mostradas; 89 delgadas omitidas). Extracción 100%; 75 relaciones inferidas con confianza promedio 0.80. Incluye `whatsapp-assistant` F1–F4 y el catálogo recuperable de Ofi. `graph.html` no se genera porque supera el límite visual de 5,000 nodos; usar `GRAPH_REPORT.md`, `graph.json` y `graphify query`. El grafo excluye cambios locales sin commit; commits solo documentales/tooling no invalidan su frescura.
