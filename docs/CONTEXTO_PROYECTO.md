...
- **Toggles privacidad** (`showPhone`/`showWhatsapp`/`showExactLocation`). **Perfil público de usuario**. **Asistente IA "Ofi"** (guardrails PII/toxicidad/inyección, caché semántico, cuota atómica). Ofi es el asistente de Servi creado por el equipo de Servi; conserva 30 mensajes durante 12 h por conversación, permite iniciar chat nuevo y responde soporte con canales directos (WhatsApp, correo y footer), sin consumir IA para intenciones conocidas. Su conocimiento estable de plataforma se compila desde `servi.md` en `servi-platform-knowledge.ts`: datos públicos curados, URLs oficiales y límites de producto, nunca Markdown/instrucciones crudas. La KB dinámica queda acotada a 30 entradas / 12 000 caracteres y no puede cambiar seguridad, alcance ni herramientas. Su aprendizaje interno usa memoria por usuario/proveedor y candidatos globales agregados, sin promover texto de chats ni PII. **Imágenes CDN**: 2 buckets R2 (público CDN + privado firmado), thumbnails Sharp.
