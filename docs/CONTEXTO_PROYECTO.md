...
**Última actualización:** 2026-08-13 · Estado: **Asistente WhatsApp F1 con OpenWA mergeado a `main`** — PR #67 (squash `667c506`), CI verde y SQL aplicado manualmente. El módulo queda aislado y apagado por feature flag hasta el smoke de salida autorizado; OpenWA conserva sesiones/envío/webhooks genéricos y Servi conserva identidad, política y auditoría mínima. Captación de negocios con botón Admin + credenciales (PRs #61/#62) sigue en producción. Subastas, ofertas, referidos/monedas, agenda y cotización continúan OCULTAS de forma reversible; Carta y Catálogo siguen visibles solo para proveedores NEGOCIO.
...
**backend/src/** — `auth` (JWT, OTP, social) · `users` · `providers` (listado/detalle/analytics/nearby) · `provider-profile` (panel propio + coverage) · `professional-migrations` (migración Oficio→Profesional preservando `Provider.id`, PR #55) · `reviews` (GPS/QR) · `favorites` · `chat` · `menu` (carta) · `catalog` · `appointments` (agenda) · `quotations` (cotización) · `coverage` (alcance por distrito) · `offer-posts` ⛔OCULTO · `subastas` ⛔OCULTO · `referrals` (canje monedas→plan) · `payments` (Yape + MercadoPago; PlanRequest DEPRECADO) · `trust-validation` · `user-reports` · `admin` (+services: dashboard/trust/reports/payments) · `ai-assistant` ("Ofi", aislado) · `whatsapp-assistant` (F1 determinista: HMAC OpenWA, dedup, STOP/HUMANO, FAQ pública; sin Ofi/IA) · `events` (WS) · `firebase` (push) · `email` · `localities` · `common` (MinioService, provider-features.service, storefront.helpers, **feature-flag.guard**) · `generated` (cliente Prisma, regenera en CI — no editar). Schema: `backend/prisma/schema.prisma`.
...
- **Asistente WhatsApp F1 con OpenWA** (PR #67, squash `667c506`, 2026-08-13): módulo `backend/src/whatsapp-assistant/`, aislado de Ofi. Recibe solo `message.received`, texto, `fromMe=false` y la sesión Servi; verifica HMAC sobre cuerpo crudo, valida sesión, rechaza grupos/otros canales, y asegura at-most-once con `unique(sessionId, HMAC(messageId))`. Nunca persiste texto, teléfono, JID ni messageId en claro. Política determinista: FAQ pública Servi, rechazo fuera de alcance, STOP persistente sin respuesta, HUMANO pausa el bot con una confirmación. Sin JWT administrativo, herramientas, pagos, perfil, referidos, OTP ni acciones de escritura. El SQL idempotente `backend/prisma/sql/whatsapp_assistant.sql` fue aplicado manualmente por el propietario. F0 OpenWA fue configurado por el propietario. Render sirve la ruta nueva y salud responde 200; el endpoint devuelve 204 con firma inválida, evidencia de `WHATSAPP_ASSISTANT_ENABLED=false`. **No activar ni ejecutar smoke con salida hasta orden explícita del propietario.** Runbook: `docs/INTEGRACION_OPENWA_SERVI.md`. F2 reutilizará Ofi en modo solo lectura; F3 vínculo OTP; F4 handover operativo/DLQ/métricas.
...
- **Asistente WhatsApp F1 mergeado, pendiente de activación (2026-08-13):** PR #67, squash `667c506`; CI Backend/Mobile/Admin y previews Vercel verdes. Verificación local: Prisma format/generate, `tsc`, build Nest, ESLint del módulo y 39 unitarias. SQL aplicado manualmente y F0 OpenWA confirmado por el propietario. Render `GET /health` = 200 y `POST /whatsapp-assistant/webhook` con cuerpo válido/firma falsa = 204: código desplegado, flag todavía apagado. Pendiente de autorización explícita: poner `WHATSAPP_ASSISTANT_ENABLED=true`, luego probar desde número de staging `FAQ → STOP → silencio`, `HUMANO → una confirmación → silencio` y firma inválida → 401. No probar con un chat real hasta esa autorización.
...
- **Graphify 2026-08-13:** regenerado y validado desde worktree limpio contra `667c506`: 1,027 archivos, 9,934 nodos, 16,168 relaciones y 474 comunidades (360 mostradas; 114 delgadas omitidas). Extracción 100%; 74 relaciones inferidas con confianza promedio 0.80. Incluye `whatsapp-assistant`. `graph.html` no se genera porque supera el límite visual de 5,000 nodos; usar `GRAPH_REPORT.md`, `graph.json` y `graphify query`. El grafo excluye cambios locales sin commit; commits solo documentales/tooling no invalidan su frescura.
