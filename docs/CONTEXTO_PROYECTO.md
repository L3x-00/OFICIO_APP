...
- **Graphify 2026-08-13:** regenerado desde `main` limpio en `0993337` (PR #69): 1,083 archivos, 9,994 nodos, 16,414 relaciones y 450 comunidades (363 mostradas; 87 delgadas omitidas). Extracción 100%; 74 relaciones inferidas con confianza promedio 0.80. Incluye `whatsapp-assistant` F1–F4. `graph.html` no se genera porque supera el límite visual de 5,000 nodos; usar `GRAPH_REPORT.md`, `graph.json` y `graphify query`. El grafo excluye cambios locales sin commit; commits solo documentales/tooling no invalidan su frescura.
