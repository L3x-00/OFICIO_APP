...
describe('MenuService (unit)', () => {
  ...
  it('carta pública: agrupa por sección y arma link de WhatsApp', async () => {
    ...
    prisma.menuItem.findMany.mockResolvedValue([
      { id: 1, name: 'Ceviche', category: 'fondo', isAvailable: true, isFeatured: false, order: 0 },
      { id: 2, name: 'Pisco Sour', category: 'bebida', isAvailable: true, isFeatured: false, order: 0 },
    ]);
  ...
  it('respeta el toggle showWhatsapp=false (sin link)', async () => {
    ...
    prisma.menuItem.findMany.mockResolvedValue([
      { id: 1, name: 'A', category: 'fondo', isAvailable: true, isFeatured: false, order: 0 },
    ]);
    const res = await service.getPublicMenu(7);
