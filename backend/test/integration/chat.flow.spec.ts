...
describe('Chat flow (integration)', () => {
  ...
  it('getOrCreateRoom es idempotente: 2 llamadas devuelven el MISMO id', async () => {
    ...
    const providerUser = await createTestUser(prisma, { email: `pu-${Date.now()}@x.com` });
  ...
  it('sendMessage persiste el mensaje y emite notificación al receptor', async () => {
    ...
    const client       = await createTestUser(prisma, { firstName: 'Cliente', lastName: 'Test' });
    const providerUser = await createTestUser(prisma, { email: `pu-${Date.now()}@x.com` });
    const provider     = await createTestProvider(prisma, providerUser.id);
    ...
    // Persistido en BD.
    const stored = await prisma.chatMessage.findUnique({ where: { id: msg.id } });
  ...
  it('markRoomAsRead marca solo los mensajes del OTRO remitente como READ', async () => {
    ...
    await service.sendMessage(client.id, { chatRoomId: room.id, senderId: client.id, content: 'A' });
    await service.sendMessage(client.id, { chatRoomId: room.id, senderId: client.id, content: 'B' });
    await service.sendMessage(providerUser.id, {
      chatRoomId: room.id, senderId: providerUser.id, content: 'C',
    });
    ...
    expect(all.filter((m) => m.senderId === client.id).every((m) => m.status === 'READ')).toBe(true);
    expect(all.find((m) => m.senderId === providerUser.id)!.status).toBe('SENT');
  });
  ...
  it('getRoomMessages devuelve paginado con hasMore correcto', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await service.sendMessage(client.id, {
        chatRoomId: room.id, senderId: client.id, content: `msg ${i + 1}`,
      });
    }

    const page1 = await service.getRoomMessages(room.id, client.id, { page: 1, limit: 3 });
    ...
    const page2 = await service.getRoomMessages(room.id, client.id, { page: 2, limit: 3 });
  ...
  it('getRoomsForUser refleja unreadCount correcto por scope', async () => {
    ...
    const room = await service.getOrCreateRoom(client.id, provider.id);
    await service.sendMessage(client.id, {
      chatRoomId: room.id, senderId: client.id, content: 'Hola',
    });
    await service.sendMessage(client.id, {
      chatRoomId: room.id, senderId: client.id, content: 'Hola otra vez',
    });

    // Para el dueño del provider, esos 2 mensajes son unread.
    const providerInbox = await service.getRoomsForUser(providerUser.id, { scope: 'provider' });
    ...
    const clientInbox = await service.getRoomsForUser(client.id, { scope: 'client' });
  ...
  it('multi-cuenta: dos clientes con el mismo provider tienen salas independientes', async () => {
    const { service } = build(prisma);
    const clientA      = await createTestUser(prisma, { email: `a-${Date.now()}@x.com` });
    const clientB      = await createTestUser(prisma, { email: `b-${Date.now()}@x.com` });
    const providerUser = await createTestUser(prisma, { email: `p-${Date.now()}@x.com` });
    const provider     = await createTestProvider(prisma, providerUser.id);
    ...
    await service.sendMessage(clientA.id, {
      chatRoomId: roomA.id, senderId: clientA.id, content: 'soy A',
    });
    await service.sendMessage(clientB.id, {
      chatRoomId: roomB.id, senderId: clientB.id, content: 'soy B',
    });

    // El proveedor ve LAS DOS salas en su inbox de provider.
    const inbox = await service.getRoomsForUser(providerUser.id, { scope: 'provider' });
