...
describe('Referrals flow (integration)', () => {
  ...
  it('getMyCode genera código único bajo demanda y es idempotente en BD', async () => {
    ...
    const rows = await prisma.referralCode.findMany({ where: { userId: inviter.id } });
  ...
  it('rechaza auto-aplicación + código inválido + doble aplicación', async () => {
    const { service } = build(prisma);
    const inviter = await createTestUser(prisma);
    const invited = await createTestUser(prisma, { email: `inv-${Date.now()}@x.com` });
    const code = await service.getMyCode(inviter.id);

    // Auto-aplicación rechazada.
    await expect(service.applyCode(inviter.id, code.code))
      .rejects.toThrow(BadRequestException);

    // Código inválido → 404.
    await expect(service.applyCode(invited.id, 'NOEXISTE'))
    ...
    await expect(service.applyCode(invited.id, code.code))
      .rejects.toThrow(ConflictException);

    // Estado final:
    const refs = await prisma.referral.findMany({ where: { invitedUserId: invited.id } });
    expect(refs).toHaveLength(1);
    expect(refs[0].status).toBe('PENDING');
    const codeRow = await prisma.referralCode.findUnique({ where: { userId: inviter.id } });
  ...
  it('onProviderApproved entrega 25 coins al inviter y 5 al invitado en BD', async () => {
    ...
    // Coins reales en BD.
    const inviterRow = await prisma.user.findUnique({ where: { id: inviter.id } });
    const invitedRow = await prisma.user.findUnique({ where: { id: invited.id } });
    ...
    const ref = await prisma.referral.findUnique({ where: { invitedUserId: invited.id } });
  ...
  it('onProviderApproved es idempotente: NO duplica coins si se llama 2 veces', async () => {
    const { service } = build(prisma);
    const inviter = await createTestUser(prisma);
    const invited = await createTestUser(prisma, { email: `inv2-${Date.now()}@x.com` });
    const code    = await service.getMyCode(inviter.id);
    await service.applyCode(invited.id, code.code);
  ...
  it('redeem de plan PREMIUM descuenta 2000 coins y activa subscription en BD', async () => {
    ...
    const sub = await prisma.subscription.findUnique({ where: { providerId: provider.id } });
    expect(sub).not.toBeNull();
    expect(sub!.plan).toBe('PREMIUM');
    expect(sub!.status).toBe('ACTIVA');

    // CoinRedemption COMPLETED.
    const redemptions = await prisma.coinRedemption.findMany({ where: { userId: user.id } });
  ...
  it('redeem de reward descuenta coins y crea CoinRedemption en PENDING', async () => {
    ...
    const ownerUser    = await createTestUser(prisma, { email: `owner-${Date.now()}@x.com` });
    ...
    const redeemer = await createTestUser(prisma, {
      email: `redeem-${Date.now()}@x.com`, coins: 500,
    });
  ...
  it('redeem rechaza si las monedas no alcanzan (sin tocar BD)', async () => {
    ...
    await expect(service.redeem(user.id, { plan: 'ESTANDAR' }))
