...
export class ReferralsService {
  ...
  async applyCode(userId: number, code: string) {
    ...
    if (existing) {
      ...
      throw new ConflictException(
        'Ya aplicaste un código de referido anteriormente desde este usuario',
      );
    }
    ...
    return rewards.map((r) => ({
      ...
      provider: r.provider
        ? {
            ...
            category: {
              name:
                r.provider.providerCategories?.[0]?.category?.name ??
                'Sin categoría',
  ...
  async redeem(userId: number, payload: { rewardId?: number; plan?: string }) {
  ...
  async getAdminStats() {
    const [total, approved, totalCoinsAgg, topInviters, monthly] =
      await Promise.all([
        this.prisma.referral.count(),
  ...
  async updateReward(
    ...
    const exists = await this.prisma.referralReward.findUnique({
      where: { id },
    });
