  ...
  async submitRequest(
    ...
    if (provider.verificationStatus !== 'APROBADO') {
      throw new BadRequestException(
        'Tu perfil debe estar aprobado antes de solicitar validación de confianza',
      );
    }
    if (provider.trustStatus === 'PENDING') {
      throw new BadRequestException(
        'Ya tienes una solicitud de validación pendiente',
      );
    }

    const uploadOpt = async (f?: Express.Multer.File) =>
      f
        ? this.minio.uploadFile(f.buffer, f.originalname, 'trust-validation')
        : undefined;
  ...
  async approveRequest(requestId: number) {
    const request = await this.prisma.trustValidationRequest.findUnique({
      ...
      select: {
        ...
        status: true,
        providerId: true,
        provider: { select: { userId: true, type: true, businessName: true } },
      },
    });
    if (!request) throw new NotFoundException('Solicitud no encontrada');
    ...
    void this.push
      .sendToUser(request.provider.userId, title, body, {
        type: 'TRUST_APPROVED',
        profileType: request.provider.type,
      })
      .catch(() => {});
