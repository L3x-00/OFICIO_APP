...
export class MercadoPagoController {
  ...
  async webhook(@Req() req: AuthenticatedRequest) {
    ...
    try {
      ...
      this.logger.log(
        `💳 Pago ${id}: status=${payment.status}, ref=${payment.externalReference}`,
      );
      ...
      this.logger.error(
        `Error procesando webhook payment=${id}: ${e instanceof Error ? e.stack : e}`,
      );
    }
  ...
  private verifySignature(req: any): boolean {
    ...
    if (!secret || secret === 'ningun_sistema' || secret.length < 16) {
      this.logger.error(
        '🛑 MERCADOPAGO_WEBHOOK_SECRET no configurado — rechazando todos los webhooks',
      );
      return false;
    }
  ...
  private async handleApprovedPayment(payment: {
    ...
    if (
      ...
      (providerType && !VALID_TYPES.has(providerType))
    ) {
    ...
    this.logger.log(
      `🎯 Activando: userId=${userId}, type=${providerType ?? 'legacy'}, plan=${plan}`,
    );
